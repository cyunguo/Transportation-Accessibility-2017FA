...
for job_density = 1:1:3
    for skill_level = 1:1:3
        ...
        [people job zone] = job_density_model_set_up(num_ppl, num_job, zone_size, job_density, skill_level);
        num_job = size(job,2);
        num_ppl = size(people,2);
%% Gravity type
% exponential
...
A_e_avg(job_density, skill_level) = mean(A_e);
...
A_cnl_avg(job_density, skill_level) = mean(A_cnl);
